...
1. 确认工作区干净，且 `package.json`、`src-tauri/Cargo.toml`、`src-tauri/tauri.conf.json` 的版本均为 `0.2.0`。
2. 在本地执行 `pnpm install --frozen-lockfile`、`pnpm test`、`pnpm build`，并在 `src-tauri` 下执行 `cargo test --locked`。
3. 提交并推送全部发布改动。
4. 创建并推送标签：`git tag v0.2.0`，然后 `git push origin v0.2.0`。
5. GitHub Actions 将验证版本、运行测试、构建 NSIS 安装包，并创建 `UNote v0.2.0` Release。
6. 发布完成后确认 Release 同时包含安装器、签名文件和 `latest.json`；缺少任意一项时不要向用户宣布自动更新可用。
7. `CHANGELOG.md` 中对应标签的章节会写入 GitHub Release 正文，并作为应用内更新弹窗的说明。
