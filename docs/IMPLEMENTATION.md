...
- 每篇笔记都属于一个可识别的笔记类型，默认是 Markdown。
- 主要内容支持 `markdown`、`excalidraw` 和 `mindmap` 三种类型，并保持统一的本地 Git 数据模型。
...
| `type`        | 笔记类型     | MUST 为 `markdown`、`excalidraw` 或 `mindmap` 之一；缺失时按 `markdown` 处理。           |
| `body`        | 正文内容     | Markdown 文本必须保留用户原始内容；非 Markdown 类型 MAY 使用结构化数据或嵌入式文本表示。 |
...
- `markdown`：普通文本笔记，默认类型，适合写作、总结和知识整理。
- `excalidraw`：白板/画板笔记，适合流程图、概念草图、结构化头脑风暴。
- `mindmap`：脑图笔记，适合主题拆解、知识发散和关系梳理。

所有类型必须共享同一套笔记本、回收站、同步和历史模型。也就是说，类型只是内容形态，不改变笔记的归属、删除流程和 Git 版本管理逻辑。
...
不同笔记类型可以使用不同的存储形式，但它们必须落在同一笔记本目录中，并在同一 Git 仓库中同步。

#### 7.2.1 Markdown 笔记

`markdown` 类型保存为 `<notebook-id>/<note-id>.md`。文件 MUST 使用 UTF-8，格式如下：
...
3. 必须识别 `title:`、`type:` 和 `updated_at:`。
4. `title` 取冒号后的去首尾空白文本。
5. `type` 允许缺失时按 `markdown` 处理；若存在，MUST 为 `markdown`。
6. `updated_at` 解析为无符号整数；无效值 MAY 降级为 `0`。
7. 文件头后的一个空行不属于正文。
8. 文件名去掉 `.md` 后得到 `note.id`。
9. 父目录名得到 `note.notebook_id`。
10. 未识别的文件头字段 SHOULD 忽略。
...
`excalidraw` 类型 MUST 保存为 `<notebook-id>/<note-id>.excalidraw.json`，其中内容为 Excalidraw 标准 JSON 数据；应用 SHOULD 同时保留 `title` 和 `updated_at` 等元信息，以便在列表中排序和展示。

为了保持可检视性，推荐在文件头保留 JSON 结构化元数据，或使用同一目录中的 Markdown 包装方式承载标题与时间字段；但不要求所有实现使用相同的外层包装格式，只要用户在 Git 中能直接找到对应绘图数据即可。

#### 7.2.3 Mindmap 笔记

`mindmap` 类型 SHOULD 保存为 `<notebook-id>/<note-id>.md`，并在文件头声明 `type: mindmap`，正文采用 Mermaid `mindmap` 语法或等价文本表示。

这种笔记仍然保持可编辑的文本格式，便于 Git 版本管理和跨平台查看；如果应用选择使用独立扩展名，如 `.mmd`，也必须保证文件仍位于笔记本目录中，并在文件头或元数据中保存 `type: mindmap`。

> 设计目标：笔记类型扩展不改变 Git 同步、回收站和版本历史的统一机制，只增加内容表达形态。
