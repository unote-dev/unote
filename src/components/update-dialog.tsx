  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" role="presentation">
      <div className="w-full max-w-sm space-y-4 rounded-lg border bg-background p-6 shadow-lg">
        <div>
          <h2 className="text-lg font-semibold">发现新版本</h2>
          <p className="mt-1 text-sm text-muted-foreground">unote v{version}</p>
        </div>
        {body && (
        ...
        {downloading && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">{total ? `${progress}%` : '正在下载…'}</p>
          </div>
        )}
        ...
        <div className="flex justify-end gap-2">
          {!downloading && (
            <>
              <Button onClick={onDismiss} type="button" variant="outline">稍后再说</Button>
              <Button onClick={onInstall} type="button">立即更新</Button>
            </>
          )}
        </div>
      </div>
